...
impl<'a> Socket<'a> {
    ...
    pub fn close(&mut self, mut interface:Interface) {
        match self.state {
    ...
    fn send_impl<'b, F, R>(&'b mut self, f: F, interface:&mut Interface) -> Result<R, SendError>
    where
    ...
    pub fn send<'b, F, R>(&'b mut self, f: F, interface:&mut Interface) -> Result<R, SendError>
    where
        F: FnOnce(&'b mut [u8]) -> (usize, R),
    {
        self.send_impl(|tx_buffer| tx_buffer.enqueue_many_with(f),interface)
    }
    ...
    pub fn send_slice(&mut self, data: &[u8], interface:&mut Interface) -> Result<usize, SendError> {
...
//     fn write_str(&mut self, slice: &str,mut interface:Interface) -> fmt::Result {
//         let slice = slice.as_bytes();
//         if self.send_slice(slice,interface) == Ok(slice.len()) {
//             Ok(())
//         } else {
//             Err(fmt::Error)
//         }
