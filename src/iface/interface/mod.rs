...
impl Interface {
    ...
    //         // 步骤1：立即尝试发送
    //         let result = self.poll_egress(now, device, sockets);
    ...
    //     }).await
    // }
    pub async fn async_poll_ingress<'a, 'b>(
        &'a mut self,
        timestamp: Instant,
        device: &mut impl Device,
        ...
        poll_fn(|cx| {
            self.inner.now = timestamp;
            match self.poll_ingress_single(timestamp, device, sockets) {
                PollIngressSingleResult::None => {
                    // 设备虽然准备好了，但实际上没有数据
                    // 可以返回 Pending 或 Ready(None)，视你的上层逻辑
                    device.register_rx_waker(cx.waker());
                    Poll::Pending
                }
                PollIngressSingleResult::PacketProcessed => {
                // 数据包被处理，但没有导致 socket 状态改变
                    self.s_waker.wake();
                    Poll::Ready(PollResult::None)
                }
                PollIngressSingleResult::SocketStateChanged => {
                    // 数据包处理导致了 socket 状态变化
                    self.s_waker.wake();
                    Poll::Ready(PollResult::SocketStateChanged)
                }
            }
        }).await
    }
    // pub async fn async_poll_ingress_loop<'a>(
    //     &'a mut self,
    //     device: &mut impl Device,
    //     sockets: &'a mut SocketSet<'a>,
    // ){
    //     loop{
    //         self.async_poll_ingress(device, sockets).await;
    //     }
    // }
    ...
    //     let item = match sockets.get_mut(handle.0) {
    //         Some(slot_opt) => match slot_opt {
    //             Some(item) => item,
    ...
    //     let mut respond = |inner: &mut InterfaceInner, meta: PacketMeta, response: Packet| {
    //         neighbor_addr = Some(response.ip_repr().dst_addr());
    //         let t = match device.transmit(inner.now) {
    //             Some(t) => t,
    //             None => {
    //                 net_debug!("failed to transmit IP: device exhausted");
    ...
    //     match result {
    //         Err(EgressError::Exhausted) => PollResult::None, // 设备满时直接返回 None 或你想的处理
    //         Err(EgressError::Dispatch) => {
    //             item.meta.neighbor_missing(
    //                 self.inner.now,
    //                 neighbor_addr.expect("non-IP response packet"),
    ...
    //         }
    //         Ok(()) => PollResult::SocketStateChanged,
    //     }
    // }
    // pub async fn async_poll_egress_single(
    //     &mut self,
    //     device: &mut (impl Device + ?Sized),
    //     sockets: &mut SocketSet<'_>,
    //     handle: SocketHandle,
    // ) -> PollResult {
    //     poll_fn(|cx| {
    //         let result = self.socket_egress_single(device, sockets, handle);
    ...
    //             socket.register_s_waker(cx.waker());
    //         }
    ...
    pub async fn wait_for_next_action(
        ...
        poll_fn(|cx| {
            if! self.polltwice{
                    self.register_s_waker(cx.waker());
                    self.polltwice=true;
                    Poll::Pending
                }
            else{
                self.polltwice=false;
                Poll::Ready(())
            }
        }).await
    }    
